 ...
 * All public API signatures are kept where possible; SPI resource extraction is
 * provided through SpbTargetInitializeWithResources() for WDF PrepareHardware.
 *
 ...
 *
 * NOTE for maintainers: in device.c OnPrepareHardware(), replace the old I2C
 * connection lookup with SpbTargetInitializeWithResources(FxDevice,
 * SpbContext, ResourcesTranslated). This code scans ACPI _CRS for
 * CM_RESOURCE_CONNECTION_TYPE_SERIAL_SPI and opens that Resource Hub path.
 */
...
static NTSTATUS
SpiGetConnectionIdFromResources(
    IN WDFCMRESLIST ResourcesTranslated,
    OUT PLARGE_INTEGER ConnectionId
    )
{
    ULONG count;
    ULONG index;

    if (ResourcesTranslated == NULL || ConnectionId == NULL)
    ...
    for (index = 0; index < count; index++)
    {
        ...
        {
            ConnectionId->LowPart = res->u.Connection.IdLowPart;
            ConnectionId->HighPart = res->u.Connection.IdHighPart;

            Trace(TRACE_LEVEL_INFORMATION, TRACE_SPB,
                "SPI ACPI connection resource found: low=0x%08lX high=0x%08lX",
                ConnectionId->LowPart,
                (ULONG)ConnectionId->HighPart);
            return STATUS_SUCCESS;
        }
    }

    Trace(TRACE_LEVEL_ERROR, TRACE_SPB,
        "No ACPI Serial SPI connection resource found in translated resources");
    ...
    )
{
    ...
    return SpiGetConnectionIdFromResources(
        ResourcesTranslated,
        &SpbContext->I2cResHubId);
}
    ...
    )
{
    ...
    if (SpbContext->I2cResHubId.LowPart == 0 &&
        SpbContext->I2cResHubId.HighPart == 0)
    {
        Trace(TRACE_LEVEL_ERROR, TRACE_SPB,
            "SPI Resource Hub ID is not configured; call SpbTargetInitializeWithResources from PrepareHardware");
        return STATUS_DEVICE_CONFIGURATION_ERROR;
    }
    ...
    )
{
    ...
    status = SpbTargetConfigureFromResources(SpbContext, ResourcesTranslated);
    if (!NT_SUCCESS(status))
        return status;

    return SpbTargetInitialize(FxDevice, SpbContext);
}
